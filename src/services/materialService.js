...
export async function getMaterials(token, { page = 1, search = '', sort = 'name_asc' } = {}) {
  ...
  return { materials: data, totalPages };
}

export async function getMaterial(token, id) {
...
export async function createMaterial(token, body) {
...
export async function updateMaterial(token, id, body) {
  const res = await fetch(`${BASE_URL}/materials/${id}`, {
    method: 'PUT',
    headers: authHeaders(token),
    body: JSON.stringify(body),
